...
void strongly_connected_components_internal(int u, const vector<vector<pair<int, int>>>& G,
    ...
    for (const auto& v : G[u]) {
        if (dfs_num[v.first] == 0)  // not visited yet
            strongly_connected_components_internal(v.first, G, comps);
...
void strongly_connected_components(const vector<vector<pair<int, int>>>& G,
    ...
    for (int i = 0; i < n; ++i)
        if (dfs_num[i] == 0)
            strongly_connected_components_internal(i, G, comps);
